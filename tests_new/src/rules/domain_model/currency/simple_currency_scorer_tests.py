...
class SimpleCurrencyScorerTests(unittest.TestCase):
    def setUp(self):
        self.nlp = TestNLP.get_nlp()
        self.sut = SimpleCurrencyScorer(['CAD', 'USD'])

    def test_simple_amount_scorer(self):
        test_contract = get_test_contract()
        test_suite = [
            ['$10.50 in USD', [('USD', 1)]], # $, NUM, curr => 1
            ['$2000 USD', [('USD', 1)]], # $, NUM, curr
            ['500 CAD', [('CAD', 0.6)]], # CAD doesnt get the MONEY entity
            ['500 USD', [('USD', 1)]], # USD DOES
            ['$555.55 CAD', [('CAD', 1)]],
            ['USD 787', [('USD', 1)]],
            ['100 chickens', []],
            ['100 1000 USD CAD $', []],
