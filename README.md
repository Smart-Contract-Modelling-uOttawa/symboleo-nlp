...
The tool generates Symboleo contracts that are based on the XText specification found at [this link](https://github.com/Smart-Contract-Modelling-uOttawa/Symboleo-IDE/blob/master/ca.uottawa.csmlab.symboleo/src/ca/uottawa/csmlab/symboleo/Symboleo.xtext). There are a few reconciliation issues between what is generated by SymboleoNLP and what is specified in XText. These may need to be addressed in future work.
- Generic Roles: The Domain objects generated by SymboleoNLP allow for the generic Role type for domain object properties (e.g., `Pay isAn Event with from: Role, to: Role;`). This generic-ness is not allowed in the XText as it currently stands. Instead, it must use specific roles (e.g., `Pay isAn Event with from: Buyer, to: Seller;`).
- There must be at least 2 parameters in the contract specification (e.g., `RentalContract(x: String, y: String)`). The isolated tests in Symboleo are only concerned with the mapping operations and many of them will not contain any parameters.
- The `Date.add()` function takes in a time value and a time unit. The XText specification does not allow for negative time values (which would semantically corresponding to subtracting from an anchor timepoint).
