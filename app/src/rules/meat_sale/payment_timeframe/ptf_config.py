...
from app.src.rules.meat_sale.payment_timeframe.ptf_matcher import get_ptf_matcher

def get_ptf_pred_config(nlp, contract_template: SymboleoContract) -> PredicateExtractorConfig:
    paid_event = contract_template.domain_model.events['paid'].to_obj()
    template = PredicateFunctionHappens(paid_event)

    matcher = get_ptf_matcher(nlp)
    
    # This is repeated from dtf_config... generalize?
    case_dict = {
    ...
    # Also duplicate from dtf config
