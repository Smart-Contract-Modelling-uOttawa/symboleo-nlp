...
def get_contract_spec(arg_dict: Dict[str,str] = arg_values):
    ...
    PAY_DATE = f'"{arg_dict["pay_date"]}"'
    START_TIME = f'"{arg_dict["start_time"]}"'
    END_TIME = f'"{arg_dict["end_time"]}"'
    ...
    contract_spec = ContractSpec(
        ...
        powers = {
            'pow_terminate_contract': Power(
                ...
                PropMaker.make(
                    PredicateFunctionHappens(
                        ObligationEvent(ObligationEventName.Violated, 'ob_dispatch')
                    ),
                    negation=True
                ),
