...
nl_template = NLTemplate(
    template_dict = {
        ...
        'payment': TemplateObj(
            'Both Parties agree that once the Parties have agreed to transaction which includes the purchase and sale of the Product that the Buyer will remit payment [PARAMETER]',
        ...
        'delivery_location': TemplateObj(
            'The point of delivery of the Product Samples shall be a laboratory determined by Gridiron [PARAMETER]',
        ...
        'legal_proceeding': TemplateObj(
            '[PARAMETER] prompt notice is required by the one to the other.', # Coreference on "one" - what to do? maybe replace manually
            ['obligations.ob_legal_proceeding_buyer', 'obligations.ob_legal_proceeding_seller']
        ),
        'return_disclose_info': TemplateObj(
            '[PARAMETER] each party shall return all copies of any such information to the other and take every reasonable measure to preclude its representatives from sharing or keeping such information.',
            [
